...
def get_earthworm_status() -> Dict[str, Any]:
    try:
        ...
        for line in output.split("\n"):
            # Skip header lines
            if any(header in line for header in ["Process  Process", "Name", "----"]):
                process_section = True
                continue
            
            if process_section and line.strip():
                parts = line.split(maxsplit=7)
                if len(parts) >= 3 and not parts[0].startswith(("Name", "Process", "-")):  # Skip header rows
                    module_name = parts[0]
                    ...
                    try:
                        pid_value = int(parts[1]) if parts[1].isdigit() else None
                    ...
                    data['module'][module_name].update({
                        ...
                        'cpu_used': parts[5] if len(parts) > 5 else None,
                        'argument': parts[7] if len(parts) > 7 else None
                    })
...
def update_metrics() -> None:
    ...
    try:
        # Set disk space if available
        if data.get("system", {}).get("disk_space") is not None:
            disk_space.set(data["system"]["disk_space"])

        for module_name, module_data in data["module"].items():
            # Set status with safer default
            status_value = 5  # Default: Status Error
            if module_data.get("status") is not None:
                status_value = {
                    ...
                    "Stop": 1,
                    "Alive": 0
                }.get(module_data["status"], 5)
            module_status.labels(module=module_name).set(status_value)

            # Calculate allocated memory safely
            allocated_memory = 0
            if module_data.get("rss") is not None and module_data.get("vsz") is not None:
                if module_data["vsz"] > 0:  # Prevent division by zero
                    allocated_memory = (module_data["rss"]/module_data["vsz"])*100
            
            # Set metrics with safe defaults
            pid.labels(module=module_name).set(
                float(module_data.get("pid", 0)) if module_data.get("pid") else 0
            )
            module_cpu_usage.labels(module=module_name).set(
                float(module_data.get("cpu_used", 0)) if module_data.get("cpu_used") else 0
            )
            module_memory_usage_allocated.labels(module=module_name).set(round(allocated_memory, 2))
            module_memory_usage_total.labels(module=module_name).set(
                float(module_data.get("memory_used", 0)) if module_data.get("memory_used") else 0
            )
            module_vsz.labels(module=module_name).set(
                float(module_data.get("vsz", 0)) if module_data.get("vsz") else 0
            )
            module_rss.labels(module=module_name).set(
                float(module_data.get("rss", 0)) if module_data.get("rss") else 0
            )
